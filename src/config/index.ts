export const NODE_ENV = process.env.NODE_ENV as "development" | "production";
export const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL as string;
export const SUPABASE_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY as string;

export const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID as string;
export const GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET as string;

export const INSTAGRAM_CLIENT_ID = process.env.INSTAGRAM_CLIENT_ID as string;
export const INSTAGRAM_CLIENT_SECRET = process.env
  .INSTAGRAM_CLIENT_SECRET as string;

export const MENTION_CLIENT_ID = process.env.MENTION_CLIENT_ID as string;
export const MENTION_CLIENT_SECRET = process.env
  .MENTION_CLIENT_SECRET as string;

export const SOCKET_URL = process.env.SOCKET_URL as string;

export const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL as string;
